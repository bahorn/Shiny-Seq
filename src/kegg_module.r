...
Kegg_module <- function(input, output, session, DE_genes,
  ...
  Enriched_Kegg <- reactive({
    ...
    if (!is.null(wgcna_output())) {
      if ((length(wgcna_output()$modules()) > 0)) {
        enrichment_main(
          "kegg", result_de(), organism(), dds.fc(),
    ...
    if (length(selected) > 0) {
      ...
      output$op_an <- renderUI({
        ...
        if (row > num) {
          if (!is.null(combo())) {
            ...
            selectInput(session$ns("ANOVA_choice"),
      ...
      output$download_Enriched_Kegg_Table <- downloadHandler(
        filename = function() {
          ...
          condition <- combo()[[row]]
          if (col == 1) {
            paste("Up regulated Kegg for ", condition, ".xlsx")
          } else if (col == 2) {
            paste("Down regulated Kegg for ", condition, ".xlsx")
      ...
      output$plot_category <- renderUI({
        textInput(session$ns("category"),
          label = h6("Enter number of categories to display"),
          value = "10"
        )
      })
      ...
      output$download_kegg_plot <- downloadHandler(
        filename = function() {
          ...
          # print(combination)
          if (col == 1) {
            paste(input$plot_k, " of Up regulated kegg for ", condition, ".svg")
          } else if (col == 2) {
            paste(input$plot_k, " of Down regulated kegg for ", condition, ".svg")
  ...
  return(list(
    Enriched_Kegg_table = reactive({
      Enriched_Kegg()[[1]]
    }),
    Enriched_Kegg_obj = reactive({
      Enriched_Kegg()[[2]]
    })
  ))
}
