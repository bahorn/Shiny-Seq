...
Module_Differential_Expression_UI <- function(id) {
  ...
  tagList(
    fluidRow(
      ...
      conditionalPanel(
        sprintf("input['%s'] >0", ns("combo")),
        uiOutput(ns("group3"))
      )
    ),
    fluidRow(
      column(2, uiOutput(ns("comb"))),
    ...
    bsModal(ns("modalhelpvalue"), "Help for p-value distribution", ns("help_pvalue"),
      size = "large",
    ...
    bsModal(ns("modalqc"), "Quality Control", ns("p-value"),
      size = "large",
      # tabsetPanel(
      tabPanel(
...
# WGCNA_matrix=NULL)
{
  ...
  output$group1 <-
    renderUI({
      ...
      selectInput(session$ns("candidates1"),
        multiple = TRUE, label = h5("Choose candidate levels for group 1"),
        choices = condition
      )
      # }
    })
  ...
  output$group2 <-
    renderUI({
      ...
      pheno <- colData(dds.fc()[[1]])
      condition <- pheno[, as.numeric(conchoice)]
      selectInput(session$ns("candidates2"),
        multiple = TRUE, label = h5("Choose candidate levels for group 2"),
        choices = condition
      )
      # }
    })
    ...
    output$group3 <-
      renderUI({
        if (!is.null(combination())) {
          ...
          print(choices)
          selectInput(session$ns("combination"),
            multiple = TRUE, label = h5("Choose comparisons"),
            choices = choices
          )
        }
      })
  ...
  output$sliders <- renderUI({
    ...
    if (!is.null(input$combination)) {
      ...
      lapply(1:num, function(i) {
        div(style = "height: 85px;", sliderInput(
          inputId = session$ns(paste0("p-value", i)), label = paste("p-value cut-off"),
  ...
  output$sliders_fc <- renderUI({
    ...
    lapply(1:num, function(i) {
      ...
      if (as.numeric(input[[paste0("hypothesis_choice", i)]]) == 1) {
        ...
        div(style = "height: 85px;", sliderInput(
          inputId = session$ns(paste0("FC_cutoff", i)), label = paste("FC cut-off"),
  ...
  output$choices <- renderUI({
    ...
    num <- length(input$combination)
    lapply(1:num, function(i) {
      div(style = "height: 95px;", radioButtons(
        inputId = session$ns(paste0("hypothesis_choice", i)), label = "Choose the hypothesis",
  ...
  output$ind <- renderUI({
    ...
    lapply(1:num, function(i) {
      div(style = "height: 95px;", radioButtons(
        inputId = session$ns(paste0("ind_choice", i)), label = "FDR correction",
        ...
        if (length(levels(dds.fc$condition)) == 2) # If there are only two treatment/condition groups in an experiment
          {
            if (input[[paste0("ind_choice", i)]] == FALSE) {
              res <- results(dds,
                pAdjustMethod = "none", # If FDR is set by user to false, then multiple testing is waived off
                altHypothesis = hyp[[hyp_choice[[i]]]],
  ...
  output$de_genes <- DT::renderDataTable({
    ...
    if (input$ok3 > 0) {
      ...
      DT::datatable(res,
        ...
        options = list(
          deferRender = TRUE, scrollX = TRUE, scrollY = 150, scroller = TRUE, dom = "Bfrtip",
    ...
    if (length(selected) > 0) {
          ...
          if ((length(wgcna_output()$modules()) > 0)) # wgcna_click()==TRUE)
            {
              ...
              if (((col == 3) && (row > length(input$combination)))) {
                ...
                DT::datatable(df,
                  class = "cell-border stripe", # as.data.frame(gene_list)
      ...
      observeEvent(input$filtered_data_rows_selected, {
        ...
        if (length(selected) > 0) {
          ...
          batch <- batch_choice()
          if (as.numeric(batch) == 1) {
            callModule(gene_count_module, "module2", NULL, reactive({
              dds.fc()[[1]]
            }), reactive({
              an_gene
            }))
            ...
            print(idx)
            callModule(gene_count_module, "module2", batch_corrected()[idx, ], reactive({
              dds.fc()[[1]]
            }), reactive({
              an_gene
            }))
      ...
      output$download_DE_genes_Table <- downloadHandler(
        filename = function() {
          if (row <= length(input$combination)) {
            ...
            if (col == 1) {
              paste("Up regulated genes for ", condition, ".xlsx")
            } else if (col == 2) {
              paste("Down regulated genes for ", condition, ".xlsx")
            } else {
              paste("DE genes for ", condition, ".xlsx")
            }
          }
          else {
            mod <- wgcna_output()$modules()
        ...
        content = function(file) {
          ...
          if (row <= length(input$combination)) {
            ...
            if (col == 1) {
              nam <- paste("Up regulated genes for ", condition)
            } else if (col == 2) {
              nam <- paste("Down regulated genes for ", condition)
            } else {
              nam <- paste("DE genes for ", condition)
            }
          }
          else {
            if (row > length(input$combination)) {
  ...
  output$p_comb <- renderUI({
    if (!is.null(combination())) {
      ...
      selectInput(session$ns("p_choice"),
        label = h5("Choose comparison"),
  ...
  return(list(
    combination = reactive({
      lapply(1:length(input$combination), function(i) input$combination[i])
    }),
    de_genes = reactive({
      DE_genes()
    }),
    p_values = reactive({
      lapply(1:length(input$combination), function(i) input[[paste0("p-value", i)]])
    }),
    hypothesis_choice = reactive({
      lapply(1:length(input$combination), function(i) input[[paste0("hypothesis_choice", i)]])
    }),
    ok3 = reactive({
      input$ok3
    })
