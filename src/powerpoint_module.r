...
powerpoint_module <- function(input, output, session,
  ...
  output$downloadppt <- downloadHandler(
    ...
    content = function(file) {
      ...
      # #boxplot_raw<-
      doc <- addPlot(doc,
        fun = function() boxplot_output(assay(dds.fc()), colData(dds.fc()), as.numeric(conchoice())),
        offx = 0.5, offy = 2, width = 6, height = 5
      )
      doc <- addParagraph(doc, "Normalized data")
      # #boxplot_norm<-
      doc <- addPlot(doc,
        fun = function() boxplot_output(normal(), colData(dds.fc()), as.numeric(conchoice())),
      ...
      if (top_norm() == 2 && top_unorm() == 2) {
        title <- "2D PCA of All genes"
      } else if (top_norm() == 1 && top_unorm() == 1) {
        title <- "2D PCA of top 500 most variable genes"
      } else {
        title <- "2D PCA"
        if (top_unorm() == 2) {
      ...
      subtitle1 <- "Raw data"
      subtitle2 <- "Normalized data"
      if (input$top_norm == 2 && input$top_unorm == 2) {
        title <- "3D PCA of All genes"
      } else if (input$top_norm == 1 && input$top_unorm == 1) {
        title <- "3D PCA of top 500 most variable genes"
      } else {
        title <- "3D PCA"
      ...
      if (!is.null(batch_choice()) && as.numeric(batch_choice() != 1)) {
        ...
        if (top_norm() == 2 && top_pca_batch == 2) {
          title <- "2D PCA of All genes"
        } else if (top_norm() == 1 && top_pca_batch == 1) {
          title <- "2D PCA of top 500 most variable genes"
        } else {
          title <- "2D PCA"
          if (top_norm() == 2) {
      ...
      if (!is.null(ok3()) && ok3() > 0) {
        ...
        for (i in 1:num)
        {
          ...
          if (nrow(df) > 0) {
            ...
            title <- NULL
            if (nrow(data) >= 10) {
              title <- paste("Top 10 Up regulated Transcription Factors", name)
            } else {
              title <- paste("All Up regulated Transcription Factors", name)
            }
            print(title)
            doc <- addTitle(doc, title)
            MyFTable <- vanilla.table(head(data, 10), add.rownames = TRUE)
          ...
          if (nrow(r) > 0) {
            ...
            title <- NULL
            if (nrow(r) >= 10) {
              title <- paste("Top 10 up regulated kegg pathways", name)
            } else {
              title <- paste("All up regulated kegg pathways", name)
            }
            print(title)
            # png(paste(title,'.png'))
          ...
          if (nrow(r) > 0) {
            ...
            title <- NULL
            if (nrow(r) >= 10) {
              title <- paste("Top 10 up regulated biological processes", name)
            } else {
              title <- paste("./All up regulated biological processes", name)
            }
            print(title)
            p <- function() {
              enrichment_plot("biological process", res_bp, result_bp, i, 1, 10, "Barplot", "")
            }
            nam <- str_replace_all(paste("./", title, sep = ""), " ", "_")
          ...
          if (nrow(r) > 0) {
            ...
            title <- NULL
            if (nrow(r) >= 10) {
              title <- paste("Top 10 up regulated Hallmark gene sets", name)
            } else {
              title <- paste("All up regulated Hallmark gene sets", name)
            }
            print(title)
            # png(paste(title,'.png'))
            nam <- str_replace_all(paste("./", title, sep = ""), " ", "_")
          ...
          if (nrow(df) > 0) {
            ...
            title <- NULL
            if (nrow(data) >= 10) {
              title <- paste("Top 10 Down regulated Transcription Factors", name)
            } else {
              title <- paste("All Down regulated Transcription Factors", name)
            }
            print(title)
            doc <- addTitle(doc, title)
            MyFTable <- vanilla.table(head(data, 10), add.rownames = TRUE)
          ...
          if (nrow(r) > 0) {
            ...
            title <- NULL
            if (nrow(r) >= 10) {
              title <- paste("Top 10 down regulated kegg pathways", name)
            } else {
              title <- paste("All down regulated kegg pathways", name)
            }
            print(title)
            # png(paste(title,'.png'))
            num <- 2
          ...
          if (nrow(r) > 0) {
            ...
            title <- NULL
            if (nrow(r) >= 10) {
              title <- paste("Top 10 down regulated biological processes", name)
            } else {
              title <- paste("All down regulated biological processes", name)
            }
            print(title)
            num <- 2
            if (nrow(r2) == 0) num <- 1
            p <- function() {
              enrichment_plot("biological process", res_bp, result_bp, i, num, 10, "Barplot", "")
            }
            nam <- str_replace_all(paste("./", title, sep = ""), " ", "_")
            ggsave(paste(nam, ".png", sep = ""), p(), dpi = 1000, height = 6, width = 12)
            doc <- addTitle(doc, title)
          ...
          if (nrow(r) > 0) {
            ...
            title <- NULL
            if (nrow(r) >= 10) {
              title <- paste("Top 10 down regulated Hallmark gene sets", name)
            } else {
              title <- paste("All down regulated Hallmark gene sets", name)
            }
            print(title)
            # png(paste(title,'.png'))
            num <- 2
  ...
  output$download_all_tables <- downloadHandler(
    ...
    content = function(fname) {
      if (ok3() > 0) {
        ...
        write.xlsx2(normal(),
          file = "normalized_table.xlsx", sheetName = "Normalized table",
        ...
        if (!is.null(batch_choice()) && as.numeric(batch_choice() != 1)) {
          ...
          write.xlsx2(batch_corrected()[[1]],
            file = "batch_corrected.xlsx", sheetName = "Batch corrected table",
            col.names = TRUE, row.names = TRUE, append = FALSE
          )
        }
