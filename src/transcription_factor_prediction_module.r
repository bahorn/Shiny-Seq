  ...
  observeEvent(input$predicted_TF_cell_clicked, {
    ...
    if (length(selected) > 0) {
      ...
      output$text_pred <- renderText({
        ...
        if (col == 1) {
          paste(nrow(df), "out  of ", length(all_pred_tf), " predicted and up regulated transcription factors are present in the dataset")
        } else if (col == 2) {
          paste(nrow(df), "out  of ", length(all_pred_tf), " predicted and down regulated transcription factors are present in the dataset")
        } else {
          paste(nrow(df), "out  of ", length(all_pred_tf), " predicted transcription factors are present in the dataset")
        }
      })
      ...
      observeEvent(input$filtered_predicted_TF_rows_selected, {
        ...
        if (length(selected) > 0) {
          ...
          if (as.numeric(batch) == 1) {
            callModule(gene_count_module, "module4", NULL, reactive({
              dds.fc()[[1]]
            }), reactive({
              an_gene
            }))
            ...
            print(idx)
            callModule(gene_count_module, "module4", batch_corrected()[idx, ], reactive({
              dds.fc()[[1]]
            }), reactive({
              an_gene
            }))
      ...
      output$download_DE_pred_TF_Table <- downloadHandler(
        filename = function() {
          ...
          condition <- combo()[[row]]
          if (col == 1) {
            paste("Up regulated predicted TF for ", condition, ".xlsx")
          } else if (col == 2) {
            paste("Down regulated predicted TF for ", condition, ".xlsx")
          } else {
            paste("All DE predicted TF for ", condition, ".xlsx")
          }
        ...
        content = function(file) {
          ...
          if (col == 1) {
            nam <- paste("Up regulated Kegg for ", condition)
          } else if (col == 2) {
            nam <- paste("Down regulated Kegg for ", condition)
          } else if (col == 3) nam <- paste("regulated Kegg for ", condition)
          write.xlsx2(df_final, file,
            sheetName = nam,
