  ...
  observeEvent(input$de_fc_cell_clicked, {
    ...
    if (nrow(selected) == 2) {
      ...
      if (col1 == 1) {
        ...
        A.idx <- which(AB_merge$padj.x < input[[paste0("p-value", row1)]])
      }
      print(length(A.idx))
      ...
      if (col2 == 1) {
        ...
        B.idx <- which(AB_merge$padj.y < input[[paste0("p-value", row2)]] & AB_merge$log2FoldChange.y < 0)
      } else {
        B.idx <- which(AB_merge$padj.y < input[[paste0("p-value", row2)]])
      }
      ...
      output$hover_info <- renderUI({
        if (!is.null(p)) {
          ...
          wellPanel(style = style, p(HTML(paste0(
            "<b> Gene name: </b>", rownames(point), "<br/>",
