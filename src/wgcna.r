...
WGCNA_module_UI <- function(id) {
  ns <- NS(id)
  tagList(
    bsModal("modalwgcna", "Sample clustering", "go_wgcna",
      size = "large",
      selectInput(ns("Choose_top"),
        label = h5("Enter input choice"),
        choices = list(
          "Consider top 5000 genes" = 1, "Consider the number of top genes specified by user" = 2,
      ...
      conditionalPanel(
        sprintf("input['%s']>0", ns("go_wgcna_start")),
    ...
    conditionalPanel(
      ...
      selectInput(ns("Choose_merge"),
        label = h5("Display heatmap of:"),
    ...
    bsModal("modal_vis", "Visualize modal", ns("vis_tab"),
      size = "large",
...
WGCNA_module <- function(input, output, session, infile, perform_voom,
    ...
    if (input$go_wgcna_start > 0) {
      ...
      if (input$cutoff_soft != "") {
        ...
        for (i in 1:length(colnames(traitData))) {
          traitData[which(condition == colnames(traitData)[i]), colnames(traitData)[i]] <- 1
        }
  ...
  observeEvent(input$go_wgcna_start, {
    output$cutoff_wgcna_ok <- renderUI({
      ...
      selectInput(
        inputId = session$ns("cutoff_wgcna"),
        label = "Select cutoff",
    ...
    output$cutoff_options <- renderUI({
      if (input$go_wgcna_start > 0) {
        ...
        if (!is.null(METree)) {
          ...
          selectInput(
            inputId = session$ns("cutoff_wgcna_dendo"),
            label = "Select cutoff",
            choices = nodes, multiple = FALSE, selected = h
          )
        }
    ...
    observeEvent(input$table_wgcna_columns_selected, {
      ...
      output$network_options <- renderUI({
        if (length(input$table_wgcna_columns_selected) > 0) {
          selectInput(session$ns("network_options"),
            label = "Choice of network",
            choices = list(
              "Network constructed usng top hub genes" = 1,
              "Network constructed using threshold" = 2
            ), selected = 2
          )
        }
      })
      ...
      if (ok3() > 0) {
        ...
        observeEvent(input$filtered_data_wgcna_rows_selected, {
          ...
          if (as.numeric(batch) == 1) {
            callModule(gene_count_module, "module6", NULL, reactive({
              dds.fc()
            }), reactive({
              an_gene
            }))
            ...
            print(idx)
            callModule(gene_count_module, "module6", batch_corrected()[idx, ], reactive({
              dds.fc()
            }), reactive({
              an_gene
            }))
