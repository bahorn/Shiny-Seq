...
Module_Raw_data_Input <- function(input, output, session) {
  ...
  t2g <- reactive({
    ...
    if (!exists("mart")) {
      org <- NULL
      if (as.numeric(input$organism == 1)) {
        org <- "hsapiens_gene_ensembl"
      } else if (as.numeric(input$organism == 2)) org <- "mmusculus_gene_ensembl"
      mart <-
        biomaRt::useMart(
          host = "jul2015.archive.ensembl.org", # "uswest.ensembl.org",#"grch37.ensembl.org",#,"ensembl.org",#
  ...
  txi <-
    reactive({
      ...
      if (as.numeric(input$filechoice) == 1 && input$kallisto_path != "") {
        ...
        Sys.sleep(0.1)
        d <- as.data.frame(tximport(sapply(
          dir(input[["kallisto_path"]]),
          function(id) {
            # file.path(input[["kallisto_path"]], id,"*.tsv")
            list.files(file.path(input[["kallisto_path"]], id), pattern = "*.tsv", full.names = TRUE)
  ...
  output$file1 <- renderUI({
    ...
    if (as.numeric(input$filechoice) == 2) {
      fileInput(session$ns("file1"),
        label = h5("Choose Expression file to upload"),
        accept = c(
          "text/csv",
  ...
  output$file2 <- renderUI({
    ...
    if (as.numeric(input$filechoice) == 2) # !is.null(input$file1) &&
      {
        req(input$file1)
        fileInput(session$ns("file2"),
          label = h5("Choose Annotation file to upload"),
          accept = c(
            "text/csv",
            "text/comma-separated-values",
            "text/tab-separated-values",
