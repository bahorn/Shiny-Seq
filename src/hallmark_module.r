...
Hallmark_module <- function(input, output, session, DE_genes,
  ...
  Enriched_hall <- reactive({
    ...
    if (!is.null(wgcna_output())) {
      if ((length(wgcna_output()$modules()) > 0)) {
        num <- length(combo())
        enrichment_main(
          "hallmark", result(), organism(), dds.fc(),
    ...
    if (length(selected) > 0) {
      ...
      output$download_Enriched_hall_Table <- downloadHandler(
        filename = function() {
          ...
          condition <- combo()[[row]]
          if (col == 1) {
            paste("Up regulated hallmark for ", condition, ".xlsx")
          } else if (col == 2) {
            paste("Down regulated hallmark for ", condition, ".xlsx")
        ...
        content = function(file) {
          ...
          if (col == 1) {
            nam <- paste("Up regulated hallmark for ", condition)
          } else if (col == 2) {
            nam <- paste("Down regulated hallmark for ", condition)
      ...
      output$plot_category_hall <- renderUI({
        textInput(session$ns("category_hall"),
  ...
  return(list(
    Enriched_hall_table = reactive({
      Enriched_hall()[[1]]
    }),
    Enriched_hall_obj = reactive({
      Enriched_hall()[[2]]
    })
  ))
}
