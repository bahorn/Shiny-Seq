  ...
  output$download_pca_svg <- downloadHandler(
    filename = function() {
      if (identical(input$plotType, "2D")) {
        if (input$top == 2) {
          paste("2D PCA of Raw data all genes.svg")
        } else {
          paste("2D PCA of Raw data top 500 genes.svg")
        }
      }

      else if (identical(input$plotType, "3D")) {
        if (input$top == 2) {
          paste("3D PCA of Raw data all genes.pdf")
        } else {
          paste("3D PCA of Raw data top 500 genes.pdf")
        }
      }
    },
