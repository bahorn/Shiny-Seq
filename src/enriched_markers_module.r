  ...
  DE_TF <- reactive({
    ...
    if (!is.null(wgcna_output())) {
      if ((length(wgcna_output()$modules()) > 0)) {
        ...
        combo <- combination()
        num <- length(combo())
        Enriched_trasncription_factors(
          TF_list(), result_DE(), num, organism(), mod, WGCNA_matrix,
          ...
          if ((length(wgcna_output()$modules()) > 0)) # input$combination))
            {
              if (row > length(combo())) {
                ...
                if (col == 3) {
                  gene_list <- result[[row]][[4]]
                } else {
                  gene_list <- result[[row]][[col]]
                }
      ...
      observeEvent(input$filtered_TF_rows_selected, {
        ...
        if (length(selected) > 0) {
          ...
          if (as.numeric(batch) == 1) {
            callModule(gene_count_module, "module3", NULL, reactive({
              dds.fc()[[1]]
            }), reactive({
              an_gene
            }))
            ...
            print(idx)
            callModule(gene_count_module, "module3", batch_corrected()[idx, ], reactive({
              dds.fc()[[1]]
            }), reactive({
              an_gene
            }))
      ...
      output$download_DE_TF_Table <- downloadHandler(
        filename = function() {
          ...
          if (row <= length(input$combination)) {
            condition <- combo()[[row]]
            if (col == 1) {
              paste("Up regulated TF for ", condition, ".xlsx")
            } else if (col == 2) {
              paste("Down regulated TF for ", condition, ".xlsx")
            } else {
              paste("All DE TF for ", condition, ".xlsx")
            }
          }
          else {
            mod <- wgcna_output()$modules()
        ...
        content = function(file) {
          ...
          if (row <= length(input$combination)) {
            ...
            if (col == 1) {
              nam <- paste("Up regulated TF for ", condition)
            } else if (col == 2) {
              nam <- paste("Down regulated TF for ", condition)
            } else {
              nam <- paste("All DE TF for ", condition)
            }
          }
          else {
            df_final <- result[[row]][[4]]
