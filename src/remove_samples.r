...
Remove_samples <- function(input, output, session, dds.fc, rld, normal) # edata=NULL,pData=NULL,
{
  ...
  output$pca_rem <- renderPlotly({
    ...
    parameters <- list()
    if (input$top_rem == 2) {
      parameters <- pca_components(assay(data), var, pheno)
    } # pca of all genes
    else {
      parameters <- pca_components(assay(data)[genes, ], var, pheno)
    } # pca of top 500 most variable genes
  ...
  v1 <- reactiveValues(
    clickBar_list = NULL,
    clickpca_list = NULL
  ) # Represents the first mouse click, if any
