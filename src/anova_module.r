  ...
  if (!is.null(DE_genes())) {
    ...
    output$ANOVA <- DT::renderDataTable({
      ...
      DT::datatable(anova,
        ...
        options = list(
          deferRender = TRUE, scrollX = TRUE, scrollY = 300, scroller = TRUE, dom = "Bfrtip",
    ...
    observeEvent(input$ANOVA_rows_selected, {
      ...
      if (as.numeric(batch) == 1) {
        callModule(gene_count_module, "module5", NULL, reactive({
          dds()
        }), reactive({
          an_gene
        }))
        ...
        print(idx)
        callModule(gene_count_module, "module5", batch_corrected()[idx, ], reactive({
          dds()
        }), reactive({
          an_gene
        }))
