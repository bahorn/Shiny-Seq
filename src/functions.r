...
boxplot_output <- function(edata, pData, col) {
  ...
  ggplot(data = melt(data), aes(as.factor(var), value, fill = as.factor(var))) +
...
pcaplot <- function(data, annotation, top, plotType, resize_factor = NULL) {
  ...
  parameters <- list()
  if (top == 2) {
    parameters <- pca_components(dat, var, pheno)
  } # pca components computed for all genes #input
  else {
    parameters <- pca_components(dat[genes, ], var, pheno)
  } # pca compnents of top 500 most variable genes
  ...
  if (length(colors[unique(df$an)]) != 0) {
    ...
    if (identical(plotType, "2D")) { # variable plotType is input obtained from user: 2d/3d
      ...
      p0 <- ggplot(df, aes(x = PCA1, y = PCA2, color = df$an)) +
        geom_point() +
        labs(x = xlab, y = ylab) +
        scale_color_manual(name = colnames(pheno)[var], values = colors[unique(df$an)]) +
        theme_bw() # 2d PCA plot
      # req(p0)
      if (!is.null(resize_factor)) {
        p <- ggplotly(p0) %>% layout(autosize = F, dragmode = "select", width = 800, height = 400)
      } else {
        p <- ggplotly(p0) %>% layout(autosize = F, width = 1000, height = 800, dragmode = "select")
      }
    } else {
      # print()
      if (!is.null(resize_factor)) {
        p <- plot_ly(x = df$PCA1, y = df$PCA2, z = df$PCA3) %>% # 3D PCA plot
          ...
          layout(autosize = F, width = 800, height = 400, scene = list(
        ...
        p <- plot_ly(x = df$PCA1, y = df$PCA2, z = df$PCA3) %>% # 3D PCA plot
          add_markers(type = "scatter3d", color = df$an, colors = unique(colors[df$an])) %>%
          layout(autosize = F, width = 1000, height = 800, scene = list(
            xaxis = list(title = xlab),
            yaxis = list(title = ylab),
            zaxis = list(title = zlab)
          ))
    ...
    if (!is.null(resize_factor)) {
      p <- plotly_empty() %>% layout(autosize = F, width = 800, height = 400)
    } else {
      p <- plotly_empty() %>% layout(autosize = F, width = 1000, height = 800)
    }
  }
...
min_samples_three <- function(dds.norm) {
  ...
  if (length(one_sample) != 0) {
    return(list(conditions[one_sample], 1))
  } # return treatment/condition groups containing only one sample
  else if (length(two_samples) != 0) {
    return(list(conditions[two_samples], 2))
  } # return treatment/condition groups containing only two samples
...
# Zoom parameter is true when user wants to zoom in,this results in resizing the plot )
{
  ...
  if (zoom == F) {
    multidensity(normal, xlab = "mean counts", xlim = c(0, 1000), col = colors)
  } else {
    multidensity(normal, xlab = "mean counts", xlim = x, ylim = y, col = colors)
  }
}
...
enrichment_main <- function(enrichment_type, result, input_organism, dds.fc, num, mod, WGCNA_matrix, c1_hallmark) {
  ...
  if (as.numeric(input_organism) == 1) {
    ...
    if (enrichment_type == "kegg") {
      ...
      human <- useMart(
        host = "jul2015.archive.ensembl.org", # "ensembl",
...
heatmap_genes <- function(heatmap_call, dds, dds.fc, rld, heat_choice,
  ...
  if (nrow(mat) > 1 && !is.null(nrow(mat))) {
    ...
    p <- pheatmap(mat,
      annotation = annotation, color = my_palette,
      main = heatmap_name,
...
plotCounts <- function(batch_corrected = NULL, dds, gene, intgroup = "condition", normalized = TRUE,
  ...
  stopifnot(returnData | all(sapply(intgroup, function(v) {
    is(
      colData(dds)[[v]],
      "factor"
    )
