...
Module_Normalized_data_UI <- function(id) {
  ...
  tagList(
    ...
    bsModal(ns("modalhelp"), "Help for error model matrix not full rank", ns("help"),
      size = "large",
...
Module_Normalized_data <- function(input, output, session, vdata, conchoice, designchoice, v_click1) {
    ...
    tryCatch(
      ...
      error = function(c) {
        ...
        if ("the model matrix is not full rank" == substr(c$message, 1, 33)) {
          c$message <- paste0("Error:The model matrix is not full rank. To know more about this please click the help button on this page.")
        } else {
          c$message
        }
        stop(c)
  ...
  output$cut_off_plot <- renderPlotly({
    ...
    # Plotly plot
    plot_ly(
      x = dat$x, y = dat$y, type = "scattergl", mode = "markers",
      hoverinfo = "text",
      text = ~ paste(
        "Cut-off:", dat$x,
  ...
  normal <- reactive({
    ...
    # print(head(norm))
    if (!is.null(norm)) {
      createAlert(session, "message", "exampleAlert1",
        title = "Message: To proceed to next step",
  ...
  observeEvent(input$normal_rows_selected, {
    ...
    print(head(df))
    callModule(gene_count_module, "module", NULL, reactive({
      dds.fc()[[1]]
    }), reactive({
      an_gene
    }))
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {
      ...
      write.xlsx2(normal(), file,
        sheetName = "Normalized table",
