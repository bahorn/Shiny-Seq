...
Module_Batch_effect_UI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      sprintf("input['%s']==2", ns("batch_choice")),
      fluidRow(column(4, uiOutput(ns("batch")))),
      actionButton(ns("helperr2"), "Help")
    ),
    conditionalPanel(
      sprintf("input['%s']==3", ns("batch_choice")),
    ...
    bsModal(ns("modalExample"), "Batch corrected PCA", ns("go"),
      size = "large", # uiOutput("plots")),
      selectInput(ns("top_pca_sva"),
        label = h5("Should PCA be computed for:"),
    ...
    bsModal(ns("modalpca"), "batch corrected PCA", ns("PCA"),
      size = "large",
      PCA_UI(ns("pca_module_batch"))
    ),
    bsModal(ns("modalhelperr2"), "Help for error model matrix not full rank", ns("helperr2"),
      size = "large",
...
Module_Batch_effect <- function(input, output, session, conchoice, designchoice, dds.fc, normal) {
  ...
  output$text <- renderUI({
    if (as.numeric(input$batch_choice) == 3) {
      ...
      if (n.sv == 0) {
        h3("No hidden batch")
      } # If there are no surrogate variables (Means no batch effec)
      else {
        print("hi")
        print(n.sv)
        ...
        print(x)
        if (n.sv == 1) {
          textInput(session$ns("num"),
            label = list(h4(x), h4("Enter number of surrogate values to be removed")),
            value = 1
          )
        } # If total number of surrogate variables is one
        ...
        else if (n.sv > 1) # If the total number of surrogate variables is more than one
          {
            if (n.sv < 6) # If total number of surrogate variables is between 1-6. the we set the default number of
              { # Surrogate variables to be removed as 1. It is adviced not to remove too many surrogate variables as it could
                ...
                selectInput(session$ns("num"),
                  label = list(h5(x), h5("Enter number of surrogate values to be removed")),
      ...
      else if (length(input$batch) == 2) # If two known variables cause batch effect
        {
          ...
          tryCatch(dds.fc <- DESeqDataSetFromMatrix(data, colData = pheno, design = full_design),
            error = function(c) {
              ...
              if ("the model matrix is not full rank" == substr(c$message, 1, 33)) {
                c$message <- paste0("The model matrix is not full rank. To know more about this please click the help button on this page.")
              } else {
                c$message
              }
              stop(c)
    ...
    # If hidden/unknown batch effect present. SVA package is used. For more information please check vignette of package
    else if (as.numeric(input$batch_choice) == 3) # && (!is.null(as.numeric(input$num))))
      {
        ...
        if (n.sv > 0) {
          ...
          if (num_sv()[[1]] > 6) {
            svall <- sva(rld, mod, mod0, n.sv = 6)
          } # 4)#change 6)
          else {
            svall <- sva(rld, mod, mod0, n.sv = num_sv()[[1]])
          } # else pick total
          print(svall$sv)
  ...
  batch_corrected <- reactive({
    ...
    withProgress(
      message = "Processing:",
      detail = "Removing batch effect .This may take a while...",
      value = 0,
      {
        ...
        if (as.numeric(input$batch_choice) == 2) #
        ...
        else if (as.numeric(input$batch_choice) == 3) # && as.numeric(input$num)>0)#(!is.null(input$num)))
          {
            ...
            if (temp == 1) {
              clean_list[[length(clean_list) + 1]] <- cleanY(assay(rld), mod, svall)
            } else {
  ...
  output$batch_corrected <- DT::renderDataTable({
    DT::datatable(batch_corrected()[[1]],
      class = "cell-border stripe",
      selection = list(mode = "single", target = "row"),
      extensions = list("Scroller" = NULL, "Buttons" = NULL),
      options = list(
        deferRender = TRUE, scrollX = TRUE, scrollY = 250, scroller = TRUE, dom = "Bfrtip",
  ...
  output$download_batch_corrected_Data <- downloadHandler(
    filename = function() {
      "batch_corrected.xlsx"
    },
    content = function(file) {
      # write.csv(batch_corrected()[[1]], file)
      write.xlsx2(batch_corrected()[[1]], file,
        sheetName = "Batch corrected data",
  ...
  observeEvent(input$batch_corrected_rows_selected, {
    ...
    if (as.numeric(input$batch_choice) == 1) {
      callModule(gene_count_module, "module1", NULL, reactive({
        dds.fc()[[1]]
      }), reactive({
        an_gene
      }))
    } else {
      callModule(gene_count_module, "module1", result[selected_row, ], reactive({
        dds.fc()[[1]]
      }), reactive({
        an_gene
      }))
  ...
  return(list(
    batch_data_for_DESeq = reactive({
      batch_design()
    }),
    batch_corrected_data = reactive({
      batch_corrected()[[1]]
    }),
    batch_choice = reactive({
      input$batch_choice
    }),
    top_batch_pca = reactive({
      b$top
    })
  ))
}
