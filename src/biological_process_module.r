...
Biological_process_module <- function(input, output, session, DE_genes,
  ...
  Enriched_BP <- reactive({
    ...
    if (!is.null(wgcna_output())) {
      if ((length(wgcna_output()$modules()) > 0)) {
        enrichment_main(
          "biological process", result(), organism(), dds.fc(),
    ...
    if (length(selected) > 0) {
      ...
      output$download_Enriched_BP_Table <- downloadHandler(
        filename = function() {
          ...
          condition <- combo()[[row]]
          if (col == 1) {
            paste("Up regulated BP for ", condition, ".xlsx")
        ...
        content = function(file) {
          ...
          if (col == 1) {
            nam <- paste("Up regulated BP for ", condition)
      ...
      output$plot_option_bp <- renderUI({
        radioButtons(session$ns("plotType_bp"), "Plot Type:", choices = c(
          "Barplot", # "Dotplot",
          "Network plot", "GO term plot"
        ), selected = "Barplot")
      })

      output$plot_category_bp <- renderUI({
        textInput(session$ns("category_bp"),
          label = h6("Enter number of categories to display"),
          value = "10"
        )
      })
      output$plot_category_go <- renderUI({
        textInput(session$ns("category_go"),
          label = h6("Enter number of GO levels to display"),
          value = ""
        )
      })
      ...
      output$download_bp_plot <- downloadHandler(
        filename = function() {
          condition <- combo()[[row]]
          if (col == 1) {
            paste(input$plotType_bp, " of Up regulated BP for ", condition, ".pdf")
  ...
  return(list(
    Enriched_BP_table = reactive({
      Enriched_BP()[[1]]
    }),
    Enriched_BP_obj = reactive({
      Enriched_BP()[[2]]
    })
  ))
}
