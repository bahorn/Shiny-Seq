  ...
  tagList(
    fluidRow(column(
      ...
      selectInput(ns("Distance"),
        label = h5("Select distance method"),
    ...
    fluidRow(column(
      3,
      selectInput(ns("Linkage"),
        label = h5("Select clustering method"),
                           ...
                           wgcna_output) # batch_choice,batch_corrected)
{
      ...
      if (length(combo()) > 0) {
        ...
        if (!is.null(wgcna_output())) {
          if (length(wgcna_output()$modules()) > 0) {
            ...
            selectInput(session$ns("heat_choice"),
  ...
  heatmap <- reactive({
    ...
    if (heatmap_call == "ANOVA") {
      ...
      heatmap_genes(
        heatmap_call, dds(), dds.fc, rld, NULL,
        result, NULL, NULL,
        num, heatmap_name,
    ...
    else # if (heatmap_call=="DE")
    {
      ...
      if (!is.null(wgcna_output())) {
        if (length(wgcna_output()$modules()) > 0) {
          ...
          h <- heatmap_genes(
            heatmap_call, NULL, dds.fc, rld, input$heat_choice,
            result,
  ...
  output$download_heatmap <- downloadHandler(
    filename = function() {
      ...
      if (heatmap_call == "ANOVA") {
        ...
        heatmap_name <- paste("Heatmap of ", heatmap_call, combo()[[choice]])
      }
