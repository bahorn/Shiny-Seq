...
ui <- navbarPage(
  ...
  navbarMenu(
    ...
    tabPanel(
      ...
      conditionalPanel(
        ...
        bsModal("modalqc", "QC for normalized data", "qc",
          size = "large",
          conditionalPanel(
            "input.ok2==0",
        ...
        bsModal("modaloutlier", "Detect and remove outlier samples in addition to identifying batch effects", "rem_samp",
          size = "large", # uiOutput("plots")),
          conditionalPanel(
            "input.ok2==0",
  ...
  tabPanel(
    ...
    fluidRow(column(
      3,
      selectInput("option_wgcna",
        label = h5("Select choice"),
        choices = list("",
          "
  ...
  navbarMenu(
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              8,
              fileInput("add_file",
                label = h3("Choose file to upload"),
                accept = c(
                  "text/csv",
    ...
    tabPanel(
      ...
      tabsetPanel(
        ...
        tabPanel(
          "Enrichment analysis-GO:Biological processes",
...
server <- function(input, output, session) {
  ...
  v <- reactiveValues(
    data = NULL, click1 = NULL, raw_pca = NULL, norm_pca = NULL, heat_de = NULL, heat_tf = NULL, heat_anova = NULL,
  ...
  observeEvent(input_data$file2(), {
    if (!is.null(input_data$pData())) {
      ...
      createAlert(session, "alert_app", "exampleAlert_app",
        title = "Oops!",
  ...
  observeEvent(input$del, {
    ...
    if (!is.null(idx)) {
      ...
      v_list <- callModule(
        Remove_samples, "module", v$dds$dds.fc()[[1]],
        v$dds$dds.fc()[[2]],
        v$dds$normal()
      )
      callModule(QC_normalized, "module", normal[[1]], zoom = TRUE)
          ...
          output$sov <- renderPlotly({
            ...
            if (length(id) != 0) {
              ...
              p <- plot_ly(df,
                ...
                layout(
                  title = "Source of variation plot",
  ...
  output$edata <- DT::renderDataTable({
    ...
    # print(head(v$data[[3]]))
    DT::datatable(v$data[[1]],
      class = "cell-border stripe",
      selection = list(target = "column"),
      extensions = list("Scroller" = NULL, "Buttons" = NULL),
      options = list(
        deferRender = TRUE, scrollX = TRUE, scrollY = 500, scroller = TRUE, dom = "Bfrtip",
  ...
  output$downloadeData <- downloadHandler(
    ...
    content = function(file) {
      ...
      write.xlsx2(v$data[[1]], file,
        sheetName = "Expression data",
  ...
  output$pData <- DT::renderDataTable({
    DT::datatable(v$data[[2]],
      class = "cell-border stripe",
      selection = list(target = "column"),
      extensions = list("Scroller" = NULL, "Buttons" = NULL),
      options = list(
        deferRender = TRUE, scrollX = TRUE, scrollY = 200, scroller = TRUE, dom = "Bfrtip",
  ...
  observeEvent(input$ok2, {
    ...
    observeEvent(input$rem_samp, {
      ...
      output$outliers <- DT::renderDataTable({
        ...
        DT::datatable(dat,
          ...
          extensions = list("Scroller" = NULL, "Buttons" = NULL),
          options = list(
            deferRender = TRUE, scrollX = TRUE, scrollY = 350, scroller = TRUE, dom = "Bfrtip",
            buttons = list()
          )
        ) # buttons = list('copy',list(extend = 'collection',
      })
    ...
    observeEvent(input$ok3, {
      ...
      v$batch <- callModule(Module_Batch_effect, "module", input$conchoice, input$designchoice, reactive({
        v$dds$dds.fc()
      }), reactive({
        v$dds$normal()
      }))
      ...
      observeEvent(input$go_wgcna, {
        ...
        if (v$batch$batch_choice() > 1) {
          ...
          v$wgcna_output <- callModule(WGCNA_module, "module",
            ...
            reactive({
              v$batch$batch_data_for_DESeq()[[1]]
            }),
            reactive({
              input$conchoice
            }),
      ...
      v$de <- callModule(
        Module_Differential_Expression, "module", input$conchoice,
        reactive({
          v$batch$batch_data_for_DESeq()
        }),
      ...
      callModule(
        FC_FC_plot_module, "module", reactive({
          v$de$de_genes
        }),
        reactive({
          v$de$combination
      ...
      v$vol <- callModule(
        ...
        reactive({
          v$de$hypothesis_choice()
        })
      )

      callModule(
        Venn_diagram_module, "module", reactive({
          v$de$de_genes
        }),
        reactive({
          v$de$combination
      ...
      v$heat_de <- callModule(
        heatmap_module, "module", "DE", NULL,
        reactive({
          v$batch
        }),
        reactive({
      ...
      observeEvent(input$add_file, {
        ...
        v$marker <- callModule(
          Enriched_markers_module, "module1", reactive({
            TF_list
          }),
          reactive({
            input_data$organism()
        ...
        callModule(
          heatmap_module, "module2", "TF", NULL,
          reactive({
            v$batch
          }),
          reactive({
      ...
      v$TF <- callModule(
        Enriched_markers_module, "module",
        reactive({
          TF_list
        }),
        reactive({
      ...
      v$heat_tf <- callModule(
        heatmap_module, "module1", "TF", NULL,
        reactive({
          v$batch
        }),
        reactive({
      ...
      v$anova <- callModule(
        ...
        reactive({
          v$batch$batch_data_for_DESeq()[[2]]
        }),
        reactive({
          v$de$combination
        }),
      ...
      v$heat_anova <- callModule(
        heatmap_module, "module3", "ANOVA",
        reactive({
          v$anova$dds()
        }),
        reactive({
      ...
      # dds.fc()
      v$bp <- callModule(
        Biological_process_module, "module", reactive({
          v$de$de_genes
        }),
        reactive({
          input_data$organism()
      ...
      v$hallmark <- callModule(
        Hallmark_module, "module", reactive({
          v$de$de_genes
        }),
        reactive({
          input_data$organism()
      ...
      callModule(
        ...
        reactive({
          v$vol$input_scale_volx()
        }), reactive({
          v$vol$input_scale_voly()
        }), reactive({
          v$vol$hypothesis_choice()
        }),
        reactive({
          v$heat_de$input_Distance()
        }), reactive({
          v$heat_de$input_Linkage()
        }),
        reactive({
          v$heat_tf$input_Distance()
        }), reactive({
          v$heat_tf$input_Linkage()
        }),
        reactive({
          v$anova$dds()
        }), reactive({
          v$heat_anova$input_Distance()
        }), reactive({
          v$heat_anova$input_Linkage()
        }),
        reactive({
          v$kegg$Enriched_Kegg_table()
        }), reactive({
          v$kegg$Enriched_Kegg_obj()
        }),
        reactive({
          v$bp$Enriched_BP_table()
        }), reactive({
          v$bp$Enriched_BP_obj()
        }),
        reactive({
          v$hallmark$Enriched_hall_table()
        }), reactive({
          v$hallmark$Enriched_hall_obj()
        })
      )
  ...
  output$file_input1 <- renderUI({
    ...
    if (as.numeric(input$option_wgcna) == 1) {
      fileInput("input_file",
        label = h5("Please upload normalized/batch corrected data"),
        accept = c(
          "text/csv",
          "text/comma-separated-values",
          "text/tab-separated-values",
  ...
  observeEvent(input$input_file, {
    ...
    if (!is.null(filepath)) {
      ...
      output$file_input2 <- renderUI({
        fileInput("input_file2",
          label = h5("Please upload annotation data"),
          accept = c(
            "text/csv",
            "text/comma-separated-values",
            "text/tab-separated-values",
  ...
  output$go_wgcna <- renderUI({
    ...
    if (as.numeric(input$option_wgcna) == 1) {
      ...
      if (as.numeric(input$pheno_table_columns_selected) > 1) {
        actionButton("go_wgcna", "GO")
      } # any variable other than sample id(assuming saple id to be the first column)
    }
